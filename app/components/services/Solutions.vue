<template>
    ...
    >
        ...
        <div class="text-center mb-16">
            <h2 class="text-3xl font-semibold text-neutral-900 sm:text-4xl">
                Nos <span class="text-gray-500">solutions</span>
            </h2>
        </div>
        ...
        >
            ...
            <div class="grid gap-6 sm:grid-cols-2">
                <UCollapsible
                    ...
                    @update:open="(isOpen) => handleToggle(index, isOpen)"
                >
                    <!-- header de la carte -->
                    <div class="relative w-full cursor-pointer p-6 text-left">
                        <div class="space-y-4">
                            <span
                                class="text-2xl font-mono font-bold text-neutral-400"
                            >
                                [{{ index + 1 }}]
                            </span>
                            ...
                            <div class="space-y-3 pb-10">
                                <h3
                                    class="text-lg font-semibold text-neutral-900 sm:text-xl"
                                >
                                    {{ solution.title }}
                                </h3>
                                <p
                                    class="text-sm text-neutral-600 sm:text-base"
                                >
    ...
    <UEmpty
        v-else
        title="Aucune solution disponible"
        description="Les solutions seront bientôt disponibles."
        class="py-16"
    />
</template>
...
const openIndex = ref<number | null>(null); // Aucun n'est ouvert par défaut

const hasSolutions = computed(() => props.solutions.length > 0);

const handleToggle = (index: number, shouldOpen: boolean) => {
    if (shouldOpen) {
        openIndex.value = index;
    } else if (openIndex.value === index) {
        openIndex.value = null; // Permet de fermer celui qui est déjà ouvert
    }
};
</script>
