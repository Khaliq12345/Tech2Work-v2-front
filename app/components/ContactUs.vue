  ...
  <section class="space-y-6">
    <div class="space-y-3">
      ...
      <p class="text-gray-600 max-w-2xl">
        Besoin d'un accompagnement ? Remplissez le formulaire pour échanger avec
        un expert Tech2Work.
      </p>
    </div>
    <UButton
      ...
      >
        <aside class="drawer-hero hidden lg:flex">
          ...
          <div class="drawer-hero-content">
            ...
            <h3 class="text-3xl font-semibold text-white mt-4">
              Construisons votre prochain projet.
            </h3>
            <p class="mt-6 text-white/70 text-sm max-w-xs">
              Décrivez votre besoin et recevez un retour personnalisé de notre
              équipe d'experts en moins de 24 heures.
            </p>
          </div>
        </aside>
        ...
        <div class="drawer-body">
          ...
          <UForm :state="form" class="drawer-form" @submit="handleSubmit">
            <div class="grid grid-cols-1 gap-6">
              ...
              <UFormField
                label="Informations utiles sur le projet"
                name="projectInfo"
                :error="errors.projectInfo || undefined"
                :ui="formFieldUi"
              >
                <template #description>
                  <p class="text-white/60 text-xs mt-1">
                    Veuillez préciser votre besoin, budget, échéance, nom du
                    décisionnaire pour un devis précis.
                  </p>
                </template>
...
const contactSchema = object({
  ...
  companyName: pipe(
    string(),
    trim(),
    minLength(1, "Le nom de l'entreprise est requis."),
  ),
  jobTitle: pipe(
    string(),
    trim(),
    minLength(1, "Veuillez préciser votre poste."),
  ),
  email: pipe(
    string(),
    trim(),
  ...
  projectInfo: pipe(
    ...
    minLength(
      10,
      "Veuillez fournir quelques détails sur votre projet (10 caractères min.).",
    ),
  ),
});
...
const handleSubmit = async () => {
  if (!validateForm()) {
    toast.add({
      ...
      description: "Merci de vérifier les champs du formulaire.",
      color: "warning",
    });
    return;
  }
